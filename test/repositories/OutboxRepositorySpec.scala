...
import models.{ DomainEvent, EventStatus, OrderCreatedEvent, OutboxEvent }
...
class OutboxRepositorySpec
  ...
  "OutboxRepository" should {
    ...
    "increment retry count and reset to PENDING" in {
      ...
      db.run(repo.incrementRetryCount(eventId, error)).futureValue
    ...
    "not overwrite DLQ events when incrementing retry count (regression test)" in {
      ...
      val affectedRows = db.run(repo.incrementRetryCount(eventId, "Should not apply")).futureValue
    ...
    "reset stale PROCESSING events to PENDING" in {
      ...
      db.run(
        sqlu"UPDATE outbox_events SET created_at = NOW() - INTERVAL '10 minutes' WHERE id = $eventId"
      ).futureValue
